...
Age = st.number_input("Возраст", min_value=18, max_value=75, value=18)
...
if not (18 <= Age <= 75):
    st.error("Возраст должен быть в диапазоне от 18 до 75 лет")

if not (0 <= Job <= 3):
    st.error("Кол-во работ должно быть неотрицательным и не более 3-х")

if not (3 <= Duration <= 72):
        st.error("Длительность должна быть между 3 и 72")
